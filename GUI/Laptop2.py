...
class Console(QtGui.QWidget):
    ...
    def onFileSystemChanged(self,path):
        """
...
#        subprocess.Popen(["ffmpeg -y -i output.raw -vf \"movie='image_overlay.png' [logo], [in]drawtext=fontfile=/usr/share/fonts/truetype/ttf-dejavu/DejaVuSerif.ttf:box=1:boxcolor=black@0.3:text='%T':fontsize=30:fontcolor=white:x=100:y=100,[logo] overlay=0:0 [out]\" -r 30 out.mp4"],shell=True)
        subprocess.Popen(["ffmpeg -y -f h264 -i output.raw -vcodec copy output.mp4 && sshpass -p 'mani123' scp manikandasriram@192.168.1.8:/home/manikandasriram/"],shell=True)
        self.qfsw.addPaths(paths)
        QtCore.QObject.connect(self.qfsw,QtCore.SIGNAL("directoryChanged(QString)"),self.onFileSystemChanged)
        QtCore.QObject.connect(self.qfsw,QtCore.SIGNAL("fileChanged(QString)"),self.onFileSystemChanged)
...
#        sys.exit(0)
    ...
    def initUI(self):
        ...
        self.qfsw = QtCore.QFileSystemWatcher(self)
        self.qfsw.addPaths(paths)
        QtCore.QObject.connect(self.qfsw,QtCore.SIGNAL("directoryChanged(QString)"),self.onFileSystemChanged)
        QtCore.QObject.connect(self.qfsw,QtCore.SIGNAL("fileChanged(QString)"),self.onFileSystemChanged)
...
#        self.title.setPixmap(QtGui.QPixmap(os.getcwd() + "/Background.png"))
        ...
        self.distance = QtGui.QLabel('Distance Travelled : ',self)
        self.distance.setFont(QtGui.QFont("Times", 30, QtGui.QFont.Bold))
        self.setStyleSheet("QLabel { font-size: 20pt; color:%s }" %  QtGui.QColor(255, 255, 255).name())
        self.distance.move(750,80)
        
        self.distance_lcd = QtGui.QLCDNumber(self)
        self.distance_lcd.move(780,100)
        self.distance_lcd.setNumDigits(4)
        self.distance_lcd.resize(150,150)
        self.distance_lcd.setFrameStyle(QtGui.QFrame.NoFrame);
        self.distance_lcd.display(0)
